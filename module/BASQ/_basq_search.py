...
from ._parameters_search import Parameters


class BASQ(Parameters):
    def __init__(self, Xobs, Yobs, prior, true_likelihood, device, dtype, quad_kwargs):
        """
        ...
        """
        super().__init__(Xobs, Yobs, prior, true_likelihood, device, dtype, quad_kwargs)
    ...
    def runtrash(self, n_batch):
        ...
        for _ in range(n_batch):
            ...
            if self.show_progress:
                EZy, VarZy = self.kq.quadrature()
                results.append([X, Y , _overhead, EZy, VarZy])
            else:
                overhead += _overhead
