...
class Parameters:
    def __init__(self, Xobs, Yobs, prior, true_likelihood, device, dtype, quad_kwargs: Optional[Dict[str, Any]]):
        """
        Args:
        ...
        """
        # BQ Modelling
        bq_model = quad_kwargs.get("bq_model", "wsabi")              # select a BQ model from ["wsabi", "vbq"], vbq stands for Vanilla BQ
        sampler_type = quad_kwargs.get("sampler_type", "prior")         # select a sampler from ["uncertainty", "prior"]
        kernel_type = quad_kwargs.get("kernel_type", "RBF")            # select a kernel from ["RBF", "Matern32", "Matern52"]

        # WSABI modelling
        wsabi_type = quad_kwargs.get("wsabi_type", "wsabim")          # select a wsabi type from ["wsabil", "wsabim"]
        alpha_factor = quad_kwargs.get("alpha_factor", 1)               # coefficient of alpha in WSABI modelling; alpha = 0.8 * min(y)

        # GP hyperparameter training with type-II MLE
        optimiser = quad_kwargs.get("optimiser", "BoTorch")          # select a optimiser from ["L-BFGS-B", "BoTorch", "Adam"], BoTorch is the slowest but the most accurate
        lik = quad_kwargs.get("lik", 1e-10)                     # centre value of GP likelihood noise.
        rng = quad_kwargs.get("rng", 10)                        # range of likelihood noise [lik/rng, lik*rng]
        train_lik = quad_kwargs.get("train_lik", False)               # flag whether or not to train likelihood noise. if False, the noise is fixed with lik
        training_iter = quad_kwargs.get("training_iter", 1000)            # maximum number of SDG interations
        thresh = quad_kwargs.get("thresh", 0.05)                   # stopping criterion. threshold = last_MLL - current_MLL
        lr = quad_kwargs.get("lr", 0.1)                        # learning rate of Adam

        # RCHQ hyperparameters
        n_rec = quad_kwargs.get("n_rec", 20000)                   # subsampling size for kernel recombination
        nys_ratio = quad_kwargs.get("nys_ratio", 1e-2)                # subsubsampling ratio for Nystrom. Number of Nystrom samples is nys_ratio * n_rec
        batch_size = quad_kwargs.get("batch_size", 100)                # batch size
        quad_ratio = quad_kwargs.get("quad_ratio", 5)                  # supersampling ratio for quadrature. Number of recombination samples is quad_ratio * n_rec

        # Uncertainty sampling
        ratio = quad_kwargs.get("ratio", 0.5)                     # mixing ratio of prior and uncertainty sampling, 0 < r < 1
        n_gaussians = quad_kwargs.get("n_gaussians", 50)                # number of Gaussians approximating the GP-modelled acquisition function
        threshold = quad_kwargs.get("threshold", 1e-5)                # threshold to cut off the insignificant Gaussians
        sampling_method = quad_kwargs.get("sampling_method", "exact")       # select sampling method from ["exact", "approx"]

        # Utility
        show_progress = quad_kwargs.get("show_progress", True)            # flag whether or not show the quadrature result over each iteration.
